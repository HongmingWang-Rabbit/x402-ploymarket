...
async function processPublish(message: MarketPublishMessage): Promise<void> {
  ...
  // Check rate limit for auto-generated markets (not user proposals)
  const isAutoGenerated = !market.source_proposal_id;
  if (isAutoGenerated) {
    ...
    if (!rateLimit.allowed) {
      logger.warn(
        ...
        'Auto-publish rate limit reached, skipping market (user proposals will still be processed)'
      );
      // Update status to indicate rate limit hit - can be retried manually later
      await sql`
        UPDATE ai_markets
    ...
    logger.info(
      { currentCount: rateLimit.currentCount, limit: rateLimit.limit },
      'Auto-publish rate limit check passed'
    );
  }
